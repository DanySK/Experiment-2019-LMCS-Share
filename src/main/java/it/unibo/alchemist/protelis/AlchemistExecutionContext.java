...
public class AlchemistExecutionContext extends AbstractExecutionContext implements SpatiallyEmbeddedDevice, LocalizedDevice, TimeAwareDevice {
    ...
    private final LoadingCache<Position, Double> cache = CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<Position, Double>() {
                ...
                public Double load(final Position dest) {
                    if (env instanceof MapEnvironment<?> && dest instanceof GeoPosition) {
    ...
    public double distanceTo(final Position target) {
    		return getDevicePosition().getDistanceTo(target);
    }
    
    public double distanceTo(final Tuple target) {
    		Position p = getDevicePosition(), t;
    		double[] coord = new double[target.size()];
    		try {
	    		for (int i=0; i<target.size(); ++i) coord[i] = (Double)target.get(i);
    		} catch (Throwable e) {
    			throw new IllegalArgumentException(target + " is not a tuple of double coordinates.", e);
    		}
    		try {
    			t = p.getClass().getConstructor(double[].class).newInstance(new Object[] {coord});
    		} catch (Throwable e1) {
    			try {
    				Class<?>[] args = new Class<?>[target.size()];
    				Object[] vals = new Object[target.size()];
    				for (int i=0; i<target.size(); ++i) {
    					args[i] = double.class;
    					vals[i] = coord[i];
    				}
    				t = p.getClass().getConstructor(args).newInstance(vals);
    			} catch (Throwable e2) {
    				throw new IllegalArgumentException(target + " is not a coordinate I can understand. I tried twice with no luck:\n" + e1.toString() + "\n" + e2.toString());
    			}
    		}
		return p.getDistanceTo(t);
    }

    public Position getDevicePosition(final int target) {
    		return env.getPosition((ProtelisNode) env.getNodeByID(target));
    }

    public Tuple getCoordinates(final int target) {
    		Position pos = env.getPosition((ProtelisNode) env.getNodeByID(target));
        return DatatypeFactory.createTuple((Object[]) ArrayUtils.toObject(pos.getCartesianCoordinates()));
    }
    
	public Object pickHood(Field f) {
		return f.getSample(getDeviceUID());
	}
    
	public Object pickHood(Object x) {
		return x;
	}
    ...
    public double nextRandomInt(final int n) {
